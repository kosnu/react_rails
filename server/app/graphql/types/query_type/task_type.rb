class Types::QueryType::TaskType < Types::BaseObject
  ...
  field :user_id, ID, null: false, resolve: ->(task, _args, _context) { task.user_id }
  field :deadline_at, String, null: true, resolve: ->(task, _args, _context) { task.deadline_at }
  field :completion_at, String, null: true, resolve: ->(task, _args, _context) { task.completion_at }
